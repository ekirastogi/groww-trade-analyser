...
export class TradeLedgerService {
  ...
  /**
   * Recompute per-trade charges with the current rate card and Groww statement totals,
   * then rebuild profiles and daily analytics. Run once after a charges-engine change.
   */
  async recalculateCharges(clientCode: string): Promise<{ updated: number; total: number }> {
    const uid = await this.auth.getDataUserId();
    if (!uid) throw new Error('Sign in to recalculate charges');

    const trades = await this.getAllTrades(clientCode);
    if (!trades.length) return { updated: 0, total: 0 };

    const { data: uploadRows, error: uploadError } = await this.supabase.client
      .from('uploads')
      .select('*')
      .eq('user_id', uid)
      .eq('client_code', clientCode);
    ...
    const statementByUpload = new Map<string, number>();
    for (const row of uploadRows ?? []) {
      const rec = rowToCamel<UploadRecord>(row);
      if (rec.id) statementByUpload.set(rec.id, rec.chargesTotal ?? 0);
    }

    const groups = new Map<string, StoredTrade[]>();
    for (const trade of trades) {
      const key = trade.uploadId || '_none';
      const list = groups.get(key);
      if (list) list.push(trade);
      else groups.set(key, [trade]);
    }

    const next: StoredTrade[] = [];
    for (const [uploadId, group] of groups) {
      const statementTotal = uploadId === '_none' ? 0 : (statementByUpload.get(uploadId) ?? 0);
      const charges = computeTradeCharges(group, this.chargesSvc, statementTotal);
      for (const [index, trade] of group.entries()) {
        const allocatedCharges = charges[index] ?? 0;
        next.push({
          ...trade,
          allocatedCharges,
    ...
    const prevByKey = new Map(trades.map((trade) => [trade.dedupeKey, trade]));
    const changed = next.filter((trade) => {
      const prev = prevByKey.get(trade.dedupeKey);
      if (!prev) return true;
      return (
        Math.abs(prev.allocatedCharges - trade.allocatedCharges) > 0.005 ||
        Math.abs(prev.netPnL - trade.netPnL) > 0.005
      );
    });

    if (changed.length) {
      await this.commitTradesInChunks(changed, uid);
      const client = (await this.clientSvc.listClients()).find((c) => c.clientCode === clientCode);
      await this.syncDerivedData(clientCode, client?.clientName ?? clientCode, { trades: next });
    }

    return { updated: changed.length, total: trades.length };
  }

  async ensureChargesEngine(): Promise<boolean> {
    const key = 'kairo-charge-engine-v4';
    if (typeof localStorage !== 'undefined' && localStorage.getItem(key) === 'done') {
      return false;
    }
    const clients = await this.clientSvc.listClients();
    for (const client of clients) {
      await this.recalculateCharges(client.clientCode);
    }

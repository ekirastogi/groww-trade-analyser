        ...
        @if (variant() === 'bar') {
          <div class="flex gap-2 overflow-x-auto pb-1">
            @for (entry of state.reportHistory(); track entry.id) {
              <button
                type="button"
                class="flex min-w-[160px] max-w-[220px] shrink-0 flex-col rounded-lg border px-3 py-2 text-left transition hover:border-kairo-300"
                [class.border-kairo-500]="state.activeHistoryId() === entry.id"
                [class.bg-kairo-50]="state.activeHistoryId() === entry.id"
          ...
          <div class="grid w-full grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
            @for (entry of state.reportHistory(); track entry.id) {
              <button
                type="button"
                class="flex min-h-[140px] flex-col rounded-xl border border-slate-200 bg-slate-50 p-4 text-left transition hover:border-kairo-300 hover:bg-kairo-50 hover:shadow-sm"
                [ngClass]="state.activeHistoryId() === entry.id ? 'border-kairo-500 bg-kairo-50 ring-1 ring-kairo-500/30' : ''"
                (click)="select(entry.id)"
              >
          ...
          @for (entry of state.reportHistory(); track entry.id) {
            <button
              ...
              [ngClass]="state.activeHistoryId() === entry.id ? 'bg-kairo-500/15 text-white' : ''"
              (click)="select(entry.id)"
            >
